...
@section
@search_summary
Feature: LSFM - Search Summary Section

  @MADLRVP-13546
  Scenario Outline: [MADLRVP-13546] Verify LSFM - the role of Correspondent appears in the report 4.1
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/search_summary/MADLRVP-13546.xml
  ...
  @MADLRVP-13552
  Scenario Outline: [MADLRVP-13552] Verify LSFM - the originator search is done by Company Name when no NMLS ID and NMLS_001 fires
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/search_summary/MADLRVP-13552.xml
    When the request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
  ...
  @MADLRVP-13751
  Scenario: [MADLRVP-13751] Verify LSFM - the suppression of names from Search Summary when no checks are done
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/search_summary/MADLRVP-13751.xml
    When the request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
  ...
  @MADLRVP-13755
  Scenario Outline: [MADLRVP-13755] Verify LSFM - decoupling 3RDP_OFAC_WANTED and LNSFFM_OPR_OFAC_WANTED / LNSFRM_OPR_OFAC_WANTED
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/search_summary/MADLRVP-13755.xml
    When the request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
  ...
  @MADLRVP-13753
  Scenario: [MADLRVP-13753] Verify LSFM - Confirmation of Searches table AND SARA section - Verify 6 relationships are correctly tied in one single request, except for one
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/search_summary/MADLRVP-13753.xml
    ...
    And the response contains xpath
      ...
      | //SearchSummaryStatus[@actorRoleText='CLOSER2']           |
      | //SearchSummaryStatus[@actorRoleText='CLOSER']            |
      | //SearchSummaryStatus[@actorRoleText='ATTORNEYCOMPANY']   |
      | //SearchSummaryStatus[@actorRoleText='ATTORNEY']          |
      | //SearchSummaryStatus[@actorRoleText='ESCROWCOMPANY']     |
      | //SearchSummaryStatus[@actorRoleText='ESCROW']            |
      | //SearchSummaryStatus[@actorRoleText='TITLECOMPANY']      |
      | //SearchSummaryStatus[@actorRoleText='TITLE']             |
      | //SearchSummaryStatus[@actorRoleText='SETTLEMENTCOMPANY'] |
      | //SearchSummaryStatus[@actorRoleText='SETTLEMENTAGENT']   |
      | //SearchSummaryStatus[@actorRoleText='Notary']            |
    And the response has xpath
      | XPath                                                                  | expected                |
  ...
  @MADLRVP-13550
  Scenario: [MADLRVP-13550] Verify LSFM - Verify SARA call when company name contains special characters: / \ ;
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/search_summary/MADLRVP-13550.xml
    When the LSFM request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
