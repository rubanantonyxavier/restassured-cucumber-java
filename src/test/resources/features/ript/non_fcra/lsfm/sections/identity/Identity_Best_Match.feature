...
@identity
@best_match
Feature: LSFM - Identity Section - Best Match

  #noinspection CucumberTableInspection
  Scenario Outline: [<request>] LSFM - New IDP_096 alert is fired when Lexis Nexis returns the verified date as <verified_date_pattern>
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/identity/<request>.xml
    ...
    Examples:
      | request       | verified_date_pattern |
      | MADLRVP-10684 | mm//YYYY              |

  @MADLRVP-10685 @IDP_013
  Scenario Outline: [MADLRVP-10685] LSFM - Regression - Fire IDP_013 when Lexis Nexis returns different mm/dd/YYYY
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/identity/MADLRVP-10685.xml
    When the request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
  ...
  @MADLRVP-10694 @hidden_tags
  Scenario: [MADLRVP-10694] LSFM - IDP_096 - Verify trigger tag
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/identity/MADLRVP-10694.xml
    When the request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
  ...
  @MADLRVP-10704 @hidden_tags
  Scenario: [MADLRVP-10704] LSFM - IDP_096 - Verify hidden tag
    Given the request XML filepath is /ript/non_fcra/lsfm/sections/identity/MADLRVP-10704.xml
    When the request is sent
    Then response content type is XML
    And successful LSFM report is received
    And the response contains xpath
