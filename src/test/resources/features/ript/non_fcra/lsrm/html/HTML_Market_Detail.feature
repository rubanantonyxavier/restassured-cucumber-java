...
@html
@market_detail
Feature: LSRM HTML Market Detail Section

  @first_section
  @MADLRVP-9314
  Scenario: [MADLRVP-9314] Verify LSRM HTML - MARKET DETAIL - First section
    Given the request XML filepath is /ript/non_fcra/lsrm/html/MADLRVP-9314.xml
    ...
    And the HTML response has xpath with IMAGE content
      | XPath                                                       | expected                                                 |
      | //div[@id='marketDetail']/img[@alt='Market Detail Section'] | /PCFRSWebElements/images/ui/lsrm/market_detail_white.gif |
  ...
  @zip_code_summary
  @MADLRVP-9315
  Scenario: [MADLRVP-9315] Verify LSRM HTML - MARKET DETAIL - ZIP CODE SUMMARY - First section
    Given the request XML filepath is /ript/non_fcra/lsrm/html/MADLRVP-9315.xml
    When the LSRM request is sent
    Then response content type is HTML
    And HTML response doesn't contain any ERROR message
    And the HTML response has xpath with IMAGE content
    ...
    And the HTML response has xpath with TEXT content
      ...
      | //div[@id='record_loan_detail']//table[@class='data grid_8_cell'] | No recent loan detail information available for this property. |

  @zip_code_summary
  @market_owner_occupancy_data
  @MADLRVP-9316
  Scenario: [MADLRVP-9316] Verify LSRM HTML - MARKET DETAIL - ZIP CODE SUMMARY - MARKET OWNER OCCUPANCY DATA
    Given the request XML filepath is /ript/non_fcra/lsrm/html/MADLRVP-9316.xml
    When the LSRM request is sent
    Then response content type is HTML
    And HTML response doesn't contain any ERROR message
    And the HTML response has xpath with IMAGE content
...
#      First table
      ...
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//td[1]     | [MATCH --> \\$\\d{1,3},\\d{1,3}] |
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//th[2]     | Minimum Value                    |
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//td[2]     | [MATCH --> \\$\\d{1,3},\\d{1,3}] |
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//th[3]     | Maximum Value                    |
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//td[3]     | [MATCH --> \\$\\d{1,3},\\d{1,3}] |
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//th[4]     | Median Value                     |
      | //div[@id='MARKET_OWNER_OCC']//table[@class='own_occ_tabbed_table']//td[4]     | [MATCH --> \\$\\d{1,3},\\d{1,3}] |
...
#      First row
  ...
  @buyers_and_sellers
  @MADLRVP-9317
  Scenario: [MADLRVP-9317] Verify LSRM HTML - MARKET DETAIL - ZIP CODE SUMMARY - BUYERS AND SELLERS
    Given the request XML filepath is /ript/non_fcra/lsrm/html/MADLRVP-9317.xml
    When the LSRM request is sent
    Then response content type is HTML
    And HTML response doesn't contain any ERROR message
    And the HTML response has xpath with IMAGE content
